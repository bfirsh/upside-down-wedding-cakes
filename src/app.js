...
const CLASSES = {
  B:   { on: true,  lo: '#9ecbf5', hi: '#14528f', w: 0.21, label: 'Class B' },
  C:   { on: true,  lo: '#f0aac4', hi: '#8e2f56', w: 0.52, label: 'Class C' },
  D:   { on: true,  lo: '#f2cf82', hi: '#8a5b00', w: 0.65, label: 'Class D' },
  E:   { on: false, lo: '#c3c2b7', hi: '#6b6a63', w: 0.38, label: 'Class E surface' },
  SUA: { on: true,  lo: '#f2a9a9', hi: '#a32d2c', w: 0.58, label: 'Restricted / MOA' }
};
...
const fadeBody = alt => 1 - 0.72 * Math.min(1, alt / 8000);
const fadeRim  = alt => 1 - 0.22 * Math.min(1, alt / 8000);

/* Rim brightness. Every volume is rimmed the same way, always — I briefly dropped
   ceiling rims above 6,500 ft on the reasoning that you never fly over a Bravo's
   lid, and it was a mistake: some boxes had a top line and others didn't, which
...
async function loadTiles() {
  ...
  // Draw what has arrived rather than waiting for the whole viewport. Combined
  // with the nearest-first sort above, the airspace you are actually looking at
  // appears almost immediately and the horizon fills in behind it. Throttled,
  // because each refresh re-tessellates every layer.
  let flushTimer = null;
  const flush = () => { flushTimer = null; counts(); refresh(); };
  const scheduleFlush = () => { if (!flushTimer) flushTimer = setTimeout(flush, 400); };
...
const rimOp  = (c, b) =>
  Math.min(0.95, RIM_BASE + RIM_GAIN * state.opacity) * fadeRim(BANDS[b] || 0);
...
function addLayers() {
  ...
  Object.keys(CLASSES).forEach(c => {
    map.addLayer({
      ...
      paint: { 'line-color': rampFor(c), 'line-width': 1.2,
...
/* ================= boot =================
   Kick the snapshot fetch off the moment this script parses. MapLibre takes well
   over a second to fire 'load' (style, first render, basemap), and chaining the
   index fetch behind it meant nothing was even requested until then — measured at
   ~2.0s before the first tile went out, on a page that was interactive at 0.3s. */
const snapshotReady = fetch('data/index.json', { cache: 'no-cache' })
  .then(r => r.ok ? r.json() : null)
  .then(j => { if (j && j.tiles && j.features) state.snapshot = j; })
  .catch(() => { /* no baked data — fall back to the live FAA service */ });

/* And don't wait for the map to fetch the tiles either. The transform is set up in
   the Map constructor, so viewBox() is answerable straight away — the fetches and
   the map's own start-up are independent work. refresh() no-ops until the style is
   up, so anything that lands early is simply drawn when addLayers() has run. */
const dataReady = snapshotReady.then(() => {
  if (!state.snapshot) return;
  try { return loadTiles(); } catch (e) { /* fall through to loadView below */ }
});
...
async function boot() {
  ...
  makeAirports();
  initGizmo();
  refresh();                 // draw whatever arrived while the map was starting
  await dataReady;
  loadView(true);            // fills any gap, or takes the live path if data/ is absent
}
// 'style.load' fires as soon as the style is parsed, well before 'load' waits on the
// first full render. Layers can be added then, so airspace appears as early as the
// data allows. Keep 'load' as a fallback in case the earlier event ever stops firing.
